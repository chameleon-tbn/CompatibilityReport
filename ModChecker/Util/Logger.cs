...
namespace ModChecker.Util
{
    ...
    {
        // Loglevel constants to make the Log calls more readable; info is only used within this class
        private  const Enums.LogLevel info    = Enums.LogLevel.Info;
        ...
        private static readonly MyFile log = new MyFile(ModSettings.LogfileFullPath, timeStamp: true, overwrite: ModSettings.LogOverwrite);
        private static readonly MyFile report = new MyFile(ModSettings.ReportTextFullPath, timeStamp: false, overwrite: true);
        ...
        private class MyFile : UnityEngine.MonoBehaviour
        {
            ...
            // Constructor
            internal MyFile(string fileFullPath, bool timeStamp, bool overwrite)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (fileSize == 0)
                        {
                            ...
                            Logger.Log($"Empty file was found: \"{ Tools.PrivacyPath(fileName) }\". This is not necessarily an error.", Logger.debug);

                            // Overwrite empty file by creating a new one
                            file = File.CreateText(fileName);
                        }
                        else if ((overwrite) || (fileSize > ModSettings.LogMaxSize))
                        {
        ...
        internal static void Log(string message, Enums.LogLevel logLevel = info, bool gameLog = false)
        {
        ...
        internal static void Report(string message, Enums.LogLevel logLevel = info)
        {
            ...
            string logPrefix = (logLevel == info) ? "" : $"{ Convert.ToString(logLevel) }: ";
